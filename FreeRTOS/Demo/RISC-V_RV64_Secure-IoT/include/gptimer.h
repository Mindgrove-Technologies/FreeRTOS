...
/**
 * @fn GPT_Show_values(GPTIMER_Config_t *gptimer)
 * 
 * @brief The function `GPT_Show_values` prints the gptimer register value.
 * 
 * @details It is used to prints the gptimer register values like clock control register, Counter register, Repeated 
 * count register and Captured counter value.
 * 
 * @param GPTIMER_Config_t The `GPTIMER_Config_t` parameter in the `GPT_Show_values` function is a struct that
 * contains several properties related to configuring a General Purpose Timer (GPT).It has the following properties: 
 * gpt_num, mode, interrupt_en, period, prescalar, dutycycle, cnt_en, capture_val, output_en.
 */
uint32_t GPT_Show_values(GPTIMER_Config_t *gptimer);
