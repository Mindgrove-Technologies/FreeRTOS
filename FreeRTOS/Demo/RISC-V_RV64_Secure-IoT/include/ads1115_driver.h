 ...
 * Name of the file                  : ads1115_driver.h
 * Brief Description of file         : Header to ADS1115 16-bit adc module
 * Name of Author                    : Kishore. J
 * Email ID                          : kishore@mindgrovetech.in
 * 
 * @file ads1115_driver.hii
 * @author Kishore. J (kishore@mindgrovetech.in)
 * @brief This is a Baremetal ADS1115 Driver's Header file for Mindgrove Silicon
 * @version 0.1
 * @date 2024-01-27
 * 
...
{
public:
  // ADS1X15(uint8_t i2c_number);

  void     reset();

  bool     begin();
  bool     isConnected(uint8_t i2c_number);
...
protected:
  ...
  bool     _writeRegister(uint8_t i2c_instance, uint8_t address, uint8_t reg, uint16_t value);
  uint16_t _readRegister(uint8_t i2c_instance, uint8_t address, uint8_t reg);
